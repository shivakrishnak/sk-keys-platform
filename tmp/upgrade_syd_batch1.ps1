# SYD Batch 1 Upgrade Script - SYD-006 through SYD-015
# Upgrades v2 files to v3.0 Master Prompt spec
...
function Upgrade-SYD {
    ...
    # 1. Replace YAML frontmatter (find second ---)
    $fmEnd = $c.IndexOf("---", 4)
    ...
    # 3. Replace Related Keywords section content
    ...
    # 4. Insert TW + ST + replace Think section
    ...
    Write-Host "Upgraded: $filename" -ForegroundColor Green
}
...
$rk006 = @"**Prerequisites (understand these first):**
...
$tw006 = @"**Reusable Engineering Principle:**
...
$rk007 = @"**Prerequisites (understand these first):**
...
$tw007 = @"**Reusable Engineering Principle:**
...
$think007 = @"**Q1.** You have 10 app servers behind a load balancer, all querying a single database. Traffic increases 5x. You add 50 more app servers. Response times stay slow (2s instead of 100ms). Why didn't adding more servers help, and what should you have done instead?
...
$rk008 = @"**Prerequisites (understand these first):**
...
$tw008 = @"**Reusable Engineering Principle:**
...
$st008 = @"A load balancer does not make a system more available — it makes an already-redundant system more efficient. If you have one server behind a load balancer, the load balancer is a single point of failure and the load balancer-server pair is less available than the server alone. High availability only emerges when at least two healthy servers exist behind the balancer. Teams routinely add load balancers to single-server setups and believe they've improved availability, when they've actually added a failure point without adding redundancy."@

$think008 = @"**Q1.** A load balancer distributes requests round-robin across 10 servers. One server becomes 10x slower (due to a memory leak). The load balancer still sends it 10% of traffic. What algorithm should you use instead, and why does it solve the problem?
...
$rk009 = @"**Prerequisites (understand these first):**
...
$tw009 = @"**Reusable Engineering Principle:**
...
$think009 = @"**Q1.** Round robin sends equal traffic to 5 servers. Server 3 has a memory leak — every request leaks 10 MB. After 1 hour, Server 3 is out of memory and crashes. Round robin kept sending it 20% of traffic despite the problem. Should you have used a different algorithm? What would it catch that round robin missed?
...
$rk010 = @"**Prerequisites (understand these first):**
...
$tw010 = @"**Reusable Engineering Principle:**
...
$think010 = @"**Q1.** Least connections routes to the server with minimum active connections. If Server 1 becomes unresponsive (hanging connections never close), its connection count rises indefinitely. LC stops routing to it (good), but those hanging connections never drain. How is this problem solved in practice — what mechanism closes them?
